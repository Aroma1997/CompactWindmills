...
public enum RotorType {
	...
	WOOD(27900, 36000, WindType.ELV, WindType.LV, 0.5F, "Wooden Rotor",
			"rotorWood", ItemRotor.class), CARBON(27901, 3456000, WindType.LV,
			WindType.HV, 0.75F, "Carbon Rotor", "rotorCarbon", ItemRotor.class), ALLOY(
			27903, 432000, WindType.MV, WindType.EV, 0.9F, "Alloy Rotor",
			"rotorAlloy", ItemRotor.class), IRIDIUM(27902, 0, WindType.HV,
			WindType.EV, 1.0F, "Iridium Rotor", "rotorIridium", ItemRotor.class), ;
	...
	private void initRotor() {
		try {
			rotor = (ItemRotor) claSS.getConstructor(int.class).newInstance(id)
					.setMinMaxTier(typeMin, typeMax).setEfficiency(efficiency)
			...
			e.printStackTrace();
			LogHelper.log(Level.WARNING, "Failed to Register Rotor: "
					+ showedName);
		}
	}
